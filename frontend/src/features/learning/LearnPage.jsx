    ...
    return (<div className="min-h-screen bg-[#fbfbfb] text-slate-900 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto space-y-10">
        ...
        {!isLoading && !hasError && (<>
            ...
            {/* 3. Varsity-Style Numbered Modules Catalog */}
            {modules.length > 0 && (<section aria-labelledby="modules-catalog-heading" className="space-y-6">
                <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                  <h2 id="modules-catalog-heading" className="text-xs font-bold uppercase tracking-widest text-slate-500">
                    MODULES
                  </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {modules.map((mod, idx) => {
                    ...
                    const isLocked = Boolean(mod.is_locked || mod.status === 'LOCKED');
                    const lockReason = mod.lock_reason;
                    return (<article key={mod.id} data-testid={`module-card-${mod.slug}`} onClick={() => !isLocked && navigate(`/learn/modules/${mod.slug}`)} className={`bg-white border border-slate-200 rounded-xl p-6 flex flex-col justify-between transition-all ${isLocked
                            ? 'opacity-85'
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            {isLocked ? (<span className="inline-flex items-center gap-1 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">
                                <Lock className="w-3 h-3 text-slate-400"/> LOCKED
                              </span>) : (<span className="text-[11px] font-medium text-slate-500 uppercase tracking-wider">
                                {mod.level || 'BEGINNER'}
                              </span>)}
                          </div>
                        ...
                        <div className="pt-6 mt-4 border-t border-slate-100 flex items-center justify-between">
                          {isLocked ? (<div className="text-xs text-slate-500 flex items-center gap-1.5">
                              <Lock className="w-3.5 h-3.5 text-slate-400 shrink-0"/>
                              <span>{lockReason || 'Complete prerequisite modules to unlock.'}</span>
                            </div>) : (<button type="button" onClick={() => navigate(`/learn/modules/${mod.slug}`)} className="inline-flex items-center gap-1.5 text-xs font-semibold text-blue-700 hover:text-blue-900 focus:ring-2 focus:ring-blue-600 focus:outline-none rounded py-1 px-1 cursor-pointer transition-colors" aria-label={`View module ${mod.title}`}>
